...
class BST(BinaryTree):
    ...
    def _iter_helper(node):
        ...
        """
        if node.left:
            yield from BST._iter_helper(node.left)
        yield node.value
        if node.right:
    ...
    def __eq__(self, t2):
        ...
        '''
        tree1 = self.to_list('inorder')
        tree2 = t2.to_list('inorder')
        for node in tree1:
            if node not in tree2:
                return False
        for node in tree2:
            if node not in tree1:
                return False
        return True
    ...
    def find_largest(self):
        ...
        '''
        if self.root is None and self.root != 0:
            raise ValueError('Nothing in tree')
        else:
        ...
        if node:
            ...
            if value == node.value:
                ...
                if not node.right:
                    ...
                    new = BST._find_smallest(node.right)
                    node.value = new
                    node.right = BST._remove(node.right, new)
                    return node
        else:
            return node
